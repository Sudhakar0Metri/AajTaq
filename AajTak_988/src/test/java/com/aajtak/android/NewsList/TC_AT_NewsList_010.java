...
public class TC_AT_NewsList_010 extends Aajtak_app_Util {
	@Test(priority = 18, description = "Description: Verify that user is able to share story by tapping on the 'Share' icon available at end of any story")
	public void AT_Newslist_010() throws InterruptedException {
		...
		waitTillElementPresent(homescreen.getSuperfastShareIcon(), 15);
		clickBtn(homescreen.getSuperfastShareIcon());
		
		isElementDisplayed(homescreen.getShareOnlyonecard());
		isElementDisplayed(homescreen.getShareallcard());
		isElementDisplayed(homescreen.getCancelStoryShare());
		clickBtn(homescreen.getCancelStoryShare());
		clickBtn(homescreen.getSuperfasttitle());
